...
vector< vector<float> > normalize(vector< vector <float> > grid) {
    ...
    //create new grid with norm vector
    for(int i =0; i < grid.size(); i++)
    {
         newGrid.push_back(vector <float>());
        for(int j =0; j < grid[i].size();j++)
        {
            newGrid[i].push_back(grid[i][j]/sum);
            printf("%3f\n",newGrid[i][j]);
        }
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
    ...
    //init 3x3 blurring window
    vector < vector <float> >window {{corner_prob,adj_prob,corner_prob},{adj_prob,cen_prob,adj_prob},{corner_prob,adj_prob,corner_prob}};

    //display blurring window
    /*
    ...
    */
    ...
    //create new empty newgrid based on grid
        ...
        for(int j =0; j < grid[i].size();j++)
            {
                ...
                for(int dx = -1; dx<2; dx ++)
                {
                    ...
                    {
                        multi = window[dx+1][dy+1];
                        //printf("multi %3f\n",multi);
                        int new_i = ((i+dy+height)%height);
                        int new_j =((j+dx+width)%width);
                        newGrid[new_i][new_j] += (float)(multi * grid_val);
...
bool close_enough(vector < vector <float> > g1, vector < vector <float> > g2) {
	...
	for (i=0; i<g1.size(); i++) {
		for (j=0; j<g1[0].size(); j++) {
			v1 = g1[i][j];
			v2 = g2[i][j];
          	//modified bug based on review comments
